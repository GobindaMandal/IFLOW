...
<div class="br-mainpanel">
  ...
  <form action="{{ Route('showpurchaseOrderSummary') }}" method="POST">
    @csrf
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="border">Delivery Point</th>
                                    <th class="border">Supplier</th>
                                    <th class="border">Total Amount (BDT)</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="border">{{ $summary->po_no }}</td>
                                    <td class="border">{{ $summary->po_date }}</td>
                                    <td class="border">{{ $summary->delivery_date }}</td> 
                                    <td class="border">{{ $summary->office_code }} {{ $summary->office_name }}</td>
                                    <td class="border">{{ $summary->supp_code }} {{ $summary->supp_name }}</td>
                                    <td class="border text-right">{{ $summary->amount }}</td>
                                    <td class="border">{{ $summary->remarks }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                            <tfoot >
                                <tr class="tfooter">
                                    ...
                                    <th></th>
                                    <th></th>
                                    @foreach($report1 as $t_amnt)
                                    <th class="text-center border">Total :</th>
                                    <th class="text-right border">{{ $t_amnt->total_amount }}</th>
                                    @endforeach
